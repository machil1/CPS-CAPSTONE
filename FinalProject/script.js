...
jobTitleArr = [];
     for (i = 1; i < JobList.length; i++) {
        var jobPosting = "";
        jobPosting += "<b>Company: </b>";
        jobPosting += "<b>" + JobList[i][1] + "</b><br>";
        //jobPosting += "<label id='title' for=\'"+JobList[i][2]+"\'>Title</label>";
        jobPosting += " <b>Title:</b> ";
        jobPosting += "<label id='title' for=\'"+JobList[i][2]+"\'>"+JobList[i][2]+"</label>";
        jobPosting += " <b><p style='white-space: pre-line'>Description:</b> ";
        jobPosting += JobList[i][3] + "</p>";
        jobPosting += "<b> Location: </b>";
        jobPosting += JobList[i][4] + "<br>";
        jobPosting += "<b>Education Level:</b> ";
        jobPosting += JobList[i][5] + "<br>";
        jobPosting += "<b><p style='white-space: pre-line'>Job Responsibilities: </b>" + JobList[i][6] + "</p>";
        jobPosting += "<b><p style='white-space: pre-line'>Job Requirements: </b>" + JobList[i][7] + "<p></span><br>";
        ...
        jobPosting+= "</select>"
        jobPosting+="<button type = 'submit' id='apply' onclick = submit() class='btn btn-success'>Submit</button></span>";
...
function submit()
{
  ...
  var resumeSelected = resume.options[resume.selectedIndex].value;
  if(resumeSelected != ''){
    $.ajax({
      url: "apply.php",
      type: "POST",
      ...
      $.ajax({
        ...
        success: function (JobApp) {
            ...
            for (i = 1; i < JobApp.length; i++){
                var jobApps ="";
                jobApps += "<h2><b>Job Application #: </b>" +JobApp[i][0] +"</h2>";
                //jobApps += "<p>Job Title: " +JobApp[i][6];
                jobApps += "<br><b>Candidate ID:</b> " +JobApp[i][1];
                jobApps += "<br><b>Name:</b> " + JobApp[i][2];
                jobApps += " " +JobApp[i][3];
                jobApps += "<br><b>Email:</b> " +JobApp[i][5]; 
                jobApps += "<br><b>Resume:</b> <a href='uploads/" +JobApp[i][4] +"' target='_blank'>view resume</a></p>";
                jobApps += "<b>Application Status:</b> Pending";
                if(firstName == 'CyberHire')
                {
                  jobApps += "<p><button class = 'btn btn-success'>Accept</button> <button class = 'btn btn-success'>Deny</button></p>";
                }
                else{
                  jobApps += "<p><button class = 'btn btn-success'>Delete Application</button>";
                }
...
$(document).ready(function(){
  ...
  $('#mytabs a[href="#accHome"]').tab('show') 
